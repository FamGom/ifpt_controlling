...
from gui.views.view_tarife import TarifeView # Wir importieren deine geniale Matrix!

# ==========================================
# DIALOG FÜR PARAMETER EINES ZEITRAUMS
# ==========================================
class ParameterBearbeitenDialog(QDialog):
    """Sammelt alle wichtigen SV-Werte für ein bestimmtes Startdatum."""
    ...
    def lade_daten(self):
        ...
        try:
            ...
            # Werte den entsprechenden SpinBoxen zuweisen
            mapping = {
            ...
            for p in params:
                if p.schluessel in mapping:
                    # Aus der DB (Dezimal) in % umwandeln, bei BBG bleibt es Euro
    ...
    def daten_speichern(self):
        ...
        try:
            ...
            # Alle 8 Parameter in die DB schreiben (Prozente wieder in Dezimal umrechnen)
            werte_dict = {
...
class SystemParameterView(QWidget):
    ...
    def setup_ui(self):
        ...
        info = QLabel("Definieren Sie hier historische Beitragsbemessungsgrenzen und SV-Anteile für die Gehaltsberechnung.")
        ...
        self.spalten = ["Gültig ab", "BBG KV/PV", "BBG RV/AV", "AG-Anteile (RV/AV/KV/PV)", "VBL", "U2"]
    ...
    def load_data(self):
        ...
        try:
            ...
            for (d,) in dates:
                ...
                bbg_kv = p_dict.get('bbg_kv_pv', 0.0)
                self.table.setItem(row, 1, QTableWidgetItem(f"{bbg_kv:,.2f} €".replace(",", "X").replace(".", ",").replace("X", ".")))
                
                bbg_rv = p_dict.get('bbg_rv_av', 0.0)
                self.table.setItem(row, 2, QTableWidgetItem(f"{bbg_rv:,.2f} €".replace(",", "X").replace(".", ",").replace("X", ".")))
...
# ==========================================
# HAUPT-CONTAINER FÜR DEN MENÜPUNKT
# ==========================================
class SystemAdminMainView(QWidget):
    """Bündelt Tarife und Systemparameter in einem aufgeräumten Reiter."""
    def __init__(self):
        super().__init__()
        layout = QVBoxLayout(self)
